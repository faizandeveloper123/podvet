...
async function ensureSuperAdminSchema(platConn, dbName) {
  ...
  // Generic key/value store for the many platform collections the Super Admin
  // console manages (coupons, features, integrations, templates, invoices…).
  await platConn.query(`CREATE TABLE IF NOT EXISTS platform_records (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    collection VARCHAR(60) NOT NULL,
    data JSON NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_records_collection (collection)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  // Successful + failed platform-staff logins (the Login Logs screen).
  await platConn.query(`CREATE TABLE IF NOT EXISTS login_logs (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    admin_id INT DEFAULT NULL,
    admin_name VARCHAR(255) DEFAULT NULL,
    ...
    user_agent VARCHAR(255) DEFAULT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_loginlog_created (created_at),
    INDEX idx_loginlog_admin (admin_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);

  // Live platform-admin sessions, so the console can force-logout a device.
  await platConn.query(`CREATE TABLE IF NOT EXISTS sessions (
  ...
  // Application / API error trail for the Error Logs screen.
  await platConn.query(`CREATE TABLE IF NOT EXISTS error_logs (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  // Backup history (metadata + status). Created via the Backups screen.
  await platConn.query(`CREATE TABLE IF NOT EXISTS backups (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  for (const [col, def] of [['is_active', 'TINYINT NOT NULL DEFAULT 1'], ['last_login', 'DATETIME DEFAULT NULL']]) {
    await ensureColumn(platConn, dbName, 'users', col, `\`${col}\` ${def}`);
  }
...
function makePlatformToken(admin, secret, sid) {
...
async function recordLogin(platConn, req, { admin, username, success }) {
  try {
    await platConn.query(
      'INSERT INTO login_logs (admin_id, admin_name, username, success, ip, user_agent) VALUES (?,?,?,?,?,?)',
      [
        admin ? admin.id : null,
        ...
        String((req && req.headers['user-agent']) || '').slice(0, 255) || null,
      ],
    );
  } catch (_) { /* logging must never break login */ }
}

async function startSession(platConn, req, admin) {
  const sid = crypto.randomBytes(16).toString('hex');
  try {
    await platConn.query(
      'INSERT INTO sessions (id, admin_id, admin_name, ip, user_agent) VALUES (?,?,?,?,?)',
      [sid, admin.id, admin.name, req ? clientIp(req) : null, String((req && req.headers['user-agent']) || '').slice(0, 255) || null],
    );
  } catch (_) { /* session table optional */ }
  return sid;
}

async function endSession(platConn, sid) {
  if (!sid) return;
  try { await platConn.query('UPDATE sessions SET revoked = 1 WHERE id = ?', [sid]); } catch (_) {}
}

// Verify a platform-staff credential pair against `platform_admins`. Used by
// the clinic app's own /api/auth/login so the one login page (served at /app)
// can also sign platform staff in and hand them off to /super-admin.
// Returns:
//   null                      → not a platform account (caller should try clinic login)
//   { blocked: '<message>' }  → matched, but disabled or wrong password
//   { token, admin }          → authenticated platform staff
async function authenticatePlatformAdmin(platConn, bcrypt, identifier, password, req) {
  if (!identifier || !password) return null;
  const [rows] = await platConn.query(
    'SELECT * FROM platform_admins WHERE username = ? OR email = ?',
    [identifier, identifier],
  );
  if (!rows.length) return null;
  const a = rows[0];
  if (!a.is_active) { await recordLogin(platConn, req, { admin: a, username: identifier, success: false }); return { blocked: 'This account is disabled' }; }
  const ok = await bcrypt.compare(password, a.password);
  if (!ok) { await recordLogin(platConn, req, { admin: a, username: identifier, success: false }); return { blocked: 'Invalid credentials' }; }
  await platConn.query('UPDATE platform_admins SET last_login = NOW() WHERE id = ?', [a.id]);
  await recordLogin(platConn, req, { admin: a, username: identifier, success: true });
  const secret = await getSecret(platConn);
  const sid = await startSession(platConn, req, a);
  return { token: makePlatformToken(a, secret, sid), admin: maskAdmin({ ...a, last_login: new Date() }) };
}

module.exports = { ensureSuperAdminSchema, bootstrapSuperAdmin, registerSuperAdmin, authenticatePlatformAdmin };
...
function registerSuperAdmin(app, deps) {
  ...
  function superAuth(req, res, next) {
    (async () => {
      try {
        ...
        if (payload.sid) {
          const [s] = await platConn.query('SELECT revoked FROM sessions WHERE id = ?', [payload.sid]);
          if (!s.length || s[0].revoked) {
            return res.status(401).json({ error: { message: 'Session ended', code: 'SUPER_ADMIN_UNAUTHORIZED' } });
          }
        }
        req.admin = payload;
        next();
  ...
  app.post('/api/super-admin/login', async (req, res) => {
    try {
      ...
      const r = await authenticatePlatformAdmin(platConn, bcrypt, identifier, password, req);
      if (!r) { await recordLogin(platConn, req, { username: identifier, success: false }); return res.status(401).json({ error: { message: 'Invalid credentials' } }); }
      if (r.blocked) return res.status(r.blocked === 'Invalid credentials' ? 401 : 403).json({ error: { message: r.blocked } });
      await platConn.query(
        'INSERT INTO audit_logs (admin_id, admin_name, action, module, ip, user_agent) VALUES (?,?,?,?,?,?)',
        [r.admin.id, r.admin.name, 'login', 'auth', clientIp(req), String(req.headers['user-agent'] || '').slice(0, 255) || null],
      ).catch(() => {});
      res.json({ token: r.token, admin: r.admin });
  ...
  app.post('/api/login', async (req, res) => {
    try {
      ...
      const pa = await authenticatePlatformAdmin(platConn, bcrypt, identifier, password, req);
      if (pa) {
        if (pa.blocked) return res.status(pa.blocked === 'Invalid credentials' ? 401 : 403).json({ error: { message: pa.blocked } });
        await platConn.query(
          'INSERT INTO audit_logs (admin_id, admin_name, action, module, ip, user_agent) VALUES (?,?,?,?,?,?)',
          [pa.admin.id, pa.admin.name, 'login', 'auth', clientIp(req), String(req.headers['user-agent'] || '').slice(0, 255) || null],
        ).catch(() => {});
        return res.json({ type: 'super_admin', token: pa.token, admin: pa.admin });
      }
  ...
  app.post('/api/super-admin/logout', superAuth, async (req, res) => {
    await endSession(platConn, req.admin.sid);
  ...
  return { superAuth, audit, collectStats, clinicCounts, toRow, clinicRow, recordLogin, endSession, startSession };
}
