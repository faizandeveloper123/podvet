...
async function authenticatePlatformAdmin(platConn, bcrypt, identifier, password) {
  ...
  const a = rows[0];
  if (!a.is_active) return { blocked: 'This account is disabled' };
  const ok = await bcrypt.compare(password, a.password);
  if (!ok) return { blocked: 'Invalid credentials' };
  await platConn.query('UPDATE platform_admins SET last_login = NOW() WHERE id = ?', [a.id]);
  const secret = await getSecret(platConn);
  return { token: makePlatformToken(a, secret), admin: maskAdmin({ ...a, last_login: new Date() }) };
}
